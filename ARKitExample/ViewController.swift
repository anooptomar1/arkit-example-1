...
class ViewController: UIViewController, ARSCNViewDelegate, UIPopoverPresentationControllerDelegate, VirtualObjectSelectionViewControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
	
    // MARK: - Main Setup & View Controller methods
    override func viewDidLoad() {
        super.viewDidLoad()
        
        // Registers current settings as the defaults
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        DispatchQueue.global().async {
            ...
            object.loadImage(selectedImage)
    ...
    // I believe this is for constant rendering
	func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
		...
		DispatchQueue.main.async {
            ...
            let results: [CIFeature] = self.detector.features(in: image)
            
            for feature in results {
                let color: UIColor = UIColor.black
                color.set()
                self.textManager.showMessage("Rect Found", autoHide: true)
                self.sceneView.draw(feature.bounds)
    ...
    // Tells the delegate that a SceneKit node's properties have been updated to match
    // the current state of its corresponding anchor.
